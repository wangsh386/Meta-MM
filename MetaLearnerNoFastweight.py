...
class MetaLearner(nn.Module):
    ...
    def forward(self, x_spt, x_spt_idx, y_spt, x_qry, x_qry_idx, y_qry):
        ...
        for i in range(task_num):
            ...
            for k in range(1, self.update_step):
                ...
                y_hat,x_list = self.net(x_spt[i],  x_spt_idx[i], params=list(self.net.parameters()))
                # loss = F.cross_entropy(y_hat, y_spt[i])
                # loss.backward()
                # self.meta_optim.step()
                # grad = torch.autograd.grad(loss, fast_weights)
                # grad = torch.autograd.grad(loss, list(self.net.parameters()))
                ...
                if k < self.update_step - 1:
                    with torch.no_grad():
                        ...
                        y_hat,x_list = self.net(x_qry[i],x_qry_idx[i],  params=list(self.net.parameters()))
    ...
    def finetunning_ROC(self, x_spt, x_spt_idx, y_spt, x_qry, x_qry_idx, y_qry):
        ...
        new_net = deepcopy(self.net)
        if hasattr(new_net, 'flatten_parameters'):
          new_net.flatten_parameters()
        y_hat = new_net(x_spt, x_spt_idx, list(new_net.parameters()), "test_transfer")
#        loss = F.cross_entropy(y_hat, y_spt)
...
#        fast_weights = list(map(lambda p: p[1] - self.base_lr * p[0], zip(grad, new_net.parameters())))
        ...
        with torch.no_grad():
            y_hat,x_list = new_net(x_qry, x_qry_idx, list(new_net.parameters()), "test_tranfer")
            y_qry_all.append(y_qry.cpu().numpy())
            y_pred_all.append(F.softmax(y_hat, dim=1)[:, 1].cpu().numpy())
