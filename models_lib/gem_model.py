...
from models_lib.compound_encoder import AtomEmbedding, BondEmbedding, BondFloatRBF
...
class GeoGNNBlock(nn.Module):
    ...
    def __init__(self, embed_dim, dropout_rate, last_act, device):
        ...
        self.gnn = GINEConv(nn.Sequential(nn.Linear(embed_dim, embed_dim * 2), nn.ReLU(), nn.Linear(embed_dim * 2, embed_dim))).to(device)
    ...
    def forward(self, atom_bond_graph, bond_angle_graph, node_id, edge_id):
        ...
        for layer_id in range(self.layer_num):
            node_hidden = self.atom_bond_block_list[layer_id](
                ...
                edge_hidden, node_id[0], edge_id[0])
